  ...
  if (strictStyleIsolation) {
    if (!supportShadowDOM) {
      ...
      if (appElement.attachShadow) {
        // 在appElement下创建shadow tree，然后返回shadowRoot
...
function getRender(appName: string, appContent: string, legacyRender?: HTMLContentRender) {
  const render: ElementRender = ({ element, loading, container }, phase) => {
    ...
    // 如果element是新来的dom节点，则把containerElement下的子节点全部清除，然后在插入element
...
export async function loadApp<T extends object>(
  ...
  // sandbox 为boolean型时，strictStyleIsolation和scopedCSS永远都是false
  // sandbox 为 { strictStyleIsolation: true }时才能开启shadow dom
